...
public class DlgHoaDon extends JDialog {
	...
	// THÀNH PHẦN BÊN TRÁI - DANH SÁCH HÓA ĐƠN
	private JTextField tfTimKiemHD;
	private JButton btnTimHD;
	private JTable tbHoaDon;
	private DefaultTableModel tmHoaDon;

	// THÀNH PHẦN BÊN PHẢI - CHI TIẾT HÓA ĐƠN (NHƯ HÌNH)
	...
	public DlgHoaDon(JFrame parent) {
		...
		// Khung ngoài bao bọc chân khí
		JPanel container = new JPanel(new BorderLayout(15, 0)); // Khoảng cách giữa 2 vùng
	...
	private JPanel khoiTaoVungDanhSach() {
		...
		JLabel lbTimKiem = new JLabel("TÌM HÓA ĐƠN:");
		lbTimKiem.setFont(fontBungeeBase.deriveFont(Font.PLAIN, 18f));
		lbTimKiem.setForeground(MAU_NAU_VIEN);

		tfTimKiemHD = new JTextField();
		tfTimKiemHD.setPreferredSize(new Dimension(300, 30));
		setupRetroInput(tfTimKiemHD);

		btnTimHD = new JButton(taoIconThuNho("/images/search.png", 20, 20));
		btnTimHD.setPreferredSize(new Dimension(40, 30));
		btnTimHD.setBackground(MAU_NEN_CHINH);
		btnTimHD.setBorder(BorderFactory.createLineBorder(MAU_NAU_VIEN, 2));

		pnTimKiem.add(lbTimKiem);
		pnTimKiem.add(tfTimKiemHD);
		pnTimKiem.add(btnTimHD);

		pnTrai.add(pnTimKiem, BorderLayout.NORTH);

		// -- Bảng Danh Sách Hóa Đơn --
		// Cột dựa theo cấu trúc class HoaDon (maHoaDon, maNhanVien, maBan, tongTien, thoiGianLap,...)
		String[] colNames = { "Mã HĐ", "Bàn", "Thu ngân", "Thời gian lập", "Thành tiền", "Trạng thái" };
		...
		tbHoaDon.getColumnModel().getColumn(1).setPreferredWidth(60);
	...
	// ==========================================
	// HỮU BẠCH HỔ: VÙNG CHI TIẾT (Y HỆT HÌNH ẢNH)
	// ==========================================
	private JPanel khoiTaoVungChiTiet() {
		JPanel pnPhai = new JPanel(new BorderLayout());
		pnPhai.setPreferredSize(new Dimension(500, 0)); // Cố định bề rộng bên phải
		pnPhai.setBackground(MAU_NEN_CHINH);
		
		// Viền đen bọc toàn bộ khung Chi tiết
		...
		// -- Bảng Chi Tiết (Danh sách món) --
		// Cột lấy cảm hứng từ hình ảnh và class ChiTietHoaDon (maSanPham, soLuong, giaBan...)
		...
		header.setFont(new Font("Tahoma", Font.BOLD, 15));
		header.setBackground(MAU_HEADER); // Nhuộm màu Bạc Xỉu
		header.setForeground(MAU_NAU_VIEN);
		header.setBorder(BorderFactory.createLineBorder(MAU_NAU_VIEN, 2));
		...
		// Canh độ rộng cột y như hình
		tbChiTiet.getColumnModel().getColumn(0).setMaxWidth(40); // STT
		tbChiTiet.getColumnModel().getColumn(1).setPreferredWidth(160); // Tên sản phẩm
		tbChiTiet.getColumnModel().getColumn(2).setMaxWidth(50); // S.L
		tbChiTiet.getColumnModel().getColumn(3).setPreferredWidth(90); // Giá bán
		tbChiTiet.getColumnModel().getColumn(4).setPreferredWidth(100); // Thành tiền
		...
		pnControls.setBorder(BorderFactory.createCompoundBorder(
		...
		cbThue = new JComboBox<>(new String[]{"0", "5", "8", "10"}); // Mặc định như hình là 0
		...
		// Nút IN HÓA ĐƠN để gọi cái DlgInHoaDon
	...
	// ==========================================
	// CÁC CHIÊU THỨC GETTER / LISTENER CƠ BẢN
	// ==========================================
	...
	public JTextField getTfTimKiemHD() {
		return tfTimKiemHD;
	}
	...
	public void addBtnTimHDListener(java.awt.event.ActionListener l) {
		btnTimHD.addActionListener(l);
	}
